  ...
  <div id="anchor">
    ...
    <a-form-model :model="orderForm" :layout="'inline'">
      <a-form-model-item>
        ...
        >
          <a-select-option
            v-for="(item, idx) in $t('mapConfig.video.scale.metric')"
            :key="idx"
            :value="idx"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { title: _ => this.mWords('pcc', 't'), dataIndex: 'pcc' },
        { title: _ => this.mWords('pcu', 't'), dataIndex: 'pcu' },
        {
  ...
  methods: {
    ...
    mWords(key, type) {
      ...
      return this.$t('video.scale.' + map[type] + '.' + key)
    },
    setDefDate() {
      var d = moment().unix()
      var h = 60 * 60
