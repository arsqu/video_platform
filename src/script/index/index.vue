<template>
  ...
  <a-config-provider :locale="locale">
    <div id="main" class="main_container">
      <a-layout class="main_box" v-if="$route.name !== '404'">
        <div :class="['aside_box', { slide: collapsed }]">
          <a-layout-sider
            width="256px"
            theme="light"
            v-model="collapsed"
          ...
          >
            ...
            <cusMenu :router="routes" :collapsed="collapsed"></cusMenu>
          </a-layout-sider>
        </div>
        <a-layout>
          <!--      <breadcrumb :breadData="getBread" slot="breadcrumb" /> -->
          <cusHeader :collapsed="collapsed" :toggleAside="toggleAside" />
          <div class="head_mid">
            <tagView :tagData="getTags" />
          </div>
          <a-layout-content class="layout_box">
...
const cusHeader = () => import('@views/module/header')
const cusMenu = () => import('@components/module/menu')
const tagView = () => import('@components/module/tagView')
...
export default {
  ...
  computed: {
    ...mapGetters(['getMenu', 'getTags', 'getBread', 'getLang']),
    locale: function() {
  ...
  methods: {
    ...
    //   menuList =
    //     menuList && typeof menuList === 'string'
    //       ? JSON.parse(menuList)
    //       : this.getMenu
    //   menuList = this.exchTree(menuList)
    ...
    // exchTree(arr) {
    //   return loop(arr, [])
    //   function loop(arr, newArr) {
    //     for (var i = 0; i < arr.length; i++) {
    //       if (arr[i].columnId === 0) {
    //         arr[i].children = []
    //         arr[i].url = '/' + arr[i].url
    //         newArr.push(arr[i])
    //       } else loops(newArr, arr[i])
    //     }
    //     return newArr
    //   }
    //   function loops(newArr, data) {
    //     for (var x = 0; x < newArr.length; x++) {
    //       if (newArr[x].id === data.columnId) {
    //         if (!newArr[x].children) data.children = []
    //         data.url = '/' + data.url
    //         newArr[x].children.push(data)
    //       } else if (newArr[x].children && newArr[x].children.length > 0) {
    //         // 如果不匹配,拿到子数据继续比对
    //         loops(newArr[x].children, data)
    //       }
    //     }
    //     return newArr
    //   }
